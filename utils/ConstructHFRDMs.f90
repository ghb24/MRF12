...
!    write(6,"(A)")          "********************************************"
    ...
    subroutine DumpRDMs()
        implicit none
        integer :: unitOneRDM,i,unitTwoRDM,j,k,l

        call FindOneRDM_spin()
        call FindTwoRDM_spin()
        
        !Now, OneRDM is nOrbBasis*2 x nOrbBasis*2 array
        !Dump it
        unitOneRDM = get_free_unit()
        open(unitOneRDM,file="OneRDM",status='unknown',action='write')

        do i=1,nOrbBasis*2
            do j=1,nOrbBasis*2
                if(abs(OneRDM(i,j)).gt.1.D-9) then
                    write(unitOneRDM,"(2I6,G25.17)") i,j,OneRDM(i,j)
                endif
            enddo
        enddo
        ...
        unitTwoRDM = get_free_unit()
        open(unitTwoRDM,file="TwoRDM",status='unknown',action='write')

        do i=1,nOrbBasis*2
            do j=1,nOrbBasis*2
                do k=1,nOrbBasis*2
                    do l=1,nOrbBasis*2
                        if(abs(TwoRDM(i,j,k,l)).gt.1.D-9) then
                            write(unitTwoRDM,"(4I6,G25.17)") i,j,k,l,TwoRDM(i,j,k,l)
                        endif
                    enddo
                enddo
            enddo
...
!        WRITE(6,*) "All Integrals read"
    ...
    subroutine fill4indarr(fu,arr,Lim)
        ...
        CALL FindMaxIndices(fu,MaxFilei,MaxFilej,MaxFilek,MaxFilel)
!        write(6,"(A,4I5)") "Index upper limits for the file are (physical notation): ",MaxFilei,MaxFilej,MaxFilek,MaxFilel
!        write(6,"(A,4I5)") "Upper limits required are: ",Lim(2,1),Lim(2,2),Lim(2,3),Lim(2,4)
...
!        write(6,'(A)') "Antisymmetrizing fg integrals - converting to spin-orbital representation..."
        ...
        !Fill with spatial orbital integrals 
        allocate(ffints(1:nOrbBasis,1:nOrbBasis,1:nOrbBasis,1:nOrbBasis),stat=ierr)
        if(ierr.ne.0) stop 'error allocating'
        ...
        allocate(antisym_ff(1:nOrbBasis*2,1:nOrbBasis*2,1:nOrbBasis*2,1:nOrbBasis*2),stat=ierr)
        if(ierr.ne.0) stop 'error allocating'
